...
<script>
  ...
  const loginWithGoogle = async () => {
    try {
      const result = await signInWithPopup(auth, provider); //팝업의 결과를 받고
      const credential = GoogleAuthProvider.credentialFromResult(result); //결과에서 인증정보를 얻고
      const token = credential.accessToken; //인증정보 안에 토큰과
